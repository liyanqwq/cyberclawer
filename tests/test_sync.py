...
def test_run_sync_cycle_calls_each_provider(monkeypatch) -> None:
    ...
    assert calls == ["avd", "hkcert", "cve", "zeroday", "govcert"]
    assert collections == ["vulnerabilities", "hkcert", "cve", "zeroday", "govcert"]
    assert browser_fallbacks == [True, False, False, False, False]
